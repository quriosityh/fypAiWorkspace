...
1. **User Authentication and Authorization**
   - Users must register using email and password or Google or social sign-in.
...
2. **Item Listing Management**
   ...
   - The system supports up to 5 photos per listing
...
- The system handles at least 30 concurrent users without degradation
...
- The database supports at least 1000 users initially
...
graph TB
    ...
    %% Presentation Layer
    subgraph VERCEL["☁️ PRESENTATION LAYER - Vercel"]
    ...
    %% Application Layer
    subgraph FLYIO["🚀 APPLICATION LAYER - Fly.io"]
...
flowchart TB
  %% System Boundary
  subgraph Boundary["StuFlux System"]
    UC_Browse((Browse Listings))
    UC_Search((Search Listings))
    UC_View((View Item Details))
    UC_CreateListing((Create Listing))
    UC_ManageListing((Manage Listing))
    UC_RequestBooking((Request Booking))
    UC_CheckAvail((Check Availability))
    UC_ConfirmReject((Confirm / Reject Booking))
    UC_UpdateCalendar((Update Calendar))
    UC_CompleteBooking((Complete Booking))
  ...
  %% Generalization (same person can act as renter or owner)
  Renter --> AuthUser
  Owner --> AuthUser

  %% Include (mandatory) relationships
  UC_RequestBooking -.->|<<include>>| UC_CheckAvail
  UC_ConfirmReject -.->|<<include>>| UC_UpdateCalendar

  %% Extend (optional/conditional) relationships
  UC_WriteReview -.->|<<extend>>| UC_CompleteBooking

  classDef actor fill:#eef2ff,stroke:#4f46e5,color:#111827
  classDef usecase fill:#ecfeff,stroke:#0ea5e9,color:#0f172a
  class Guest,AuthUser,Renter,Owner actor
  class UC_Browse,UC_Search,UC_View,UC_CreateListing,UC_ManageListing,UC_RequestBooking,UC_CheckAvail,UC_ConfirmReject,UC_UpdateCalendar,UC_CompleteBooking,UC_SendMessage,UC_WriteReview usecase
```
